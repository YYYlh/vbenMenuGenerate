...
program
  ...
  .option("-m --modal [modal]", "生成带弹框的表格页面")
  .option("-d --drawer [drawer]", "生成带抽屉的表格页面")
  .action((name, args) => {
    if (!args.path) {
    ...
    let getTablePage = getTableTemplate.bind(
      null,
      name,
      upperCaseName,
      args.path
    ...
    if (args.modal) {
      fs.writeFileSync(
        pagePath + "/modal.vue",
        getModalTemplate(name, upperCaseName, args.path)
      );
      getTablePage = getTableModalTemplate.bind(
        null,
        name,
        upperCaseName,
        args.path
      ...
      fs.writeFileSync(
        pagePath + "/drawer.vue",
        getDrawerTemplate(name, upperCaseName, args.path)
      );
      getTablePage = getTableDrawerTemplate.bind(
        null,
        name,
        upperCaseName,
        args.path
    ...
    fs.mkdir(apiPath, { recursive: true }, () => {
      fs.writeFileSync(
        apiPath + `/${name}.ts`,
        getApiTemplate(name, upperCaseName)
      );
      fs.mkdir(apiPath + "/model", { recursive: true }, () => {
        fs.writeFileSync(
          apiPath + "/model" + `/${name}Model.ts`,
          getModelTemplate(name)
        );
      });
    });
  });
